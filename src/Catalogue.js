...
const Catalogue = ({ onClick }) => {
  ...
  const incrBlackSquare = useCallback(
    (e) => {
      e.stopPropagation();
  ...
  const incrWhiteSquare = useCallback(
    (e) => {
      e.stopPropagation();
      const incrAmount = Number(e.target.dataset.incr);
  ...
  const onPatternClick = useCallback(
    (e) => {
      onClick({
        pattern,
      });
